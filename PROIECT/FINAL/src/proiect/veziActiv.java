...
public class veziActiv {
	...
	private void initialize(String profil, String nume, String CNP) {
		...
		textArea = new JTextArea("Activitatile programate");
	...
	private void buttons(String profil, String nume, String CNP) {
		...
		butonSend.addActionListener(e -> {
			grades(profil, nume, CNP);

		});
		frame.getContentPane().add(butonSend);
	}

	public void grades(String profil, String nume, String CNP) {
		String curs = new String();
		String[] det = new String[1000];
		int contor = -1;
		try {
			Statement s = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
			ResultSet rs = s.executeQuery("call proiect.veziActiv('" + CNP + "')");
			...
			if (rows == 0)
				System.out.println("Nu ai activitati programate");
			else {
				rs.beforeFirst();
				while (rs.next())
