...
public class insertUser {
	...
	public insertUser(String profil, String nume, String CNP) {
		connection();
	...
	private void textFields() {
		...
		textField_9.setBounds(309, 39, 139, 30);
		panel.add(textField_9);

		textField_10 = new JTextField();
		textField_10.setColumns(10);
		textField_10.setBounds(309, 80, 139, 30);
	...
	private void labels(String profil) {
		...
		label_9 = new JLabel("An studiu:");
		label_9.setFont(new Font("Times new roman", Font.ITALIC, 14));
		label_9.setBounds(250, 46, 464, 14);
		panel.add(label_9);

		label_10 = new JLabel("Nr ore:");
		label_10.setFont(new Font("Times new roman", Font.ITALIC, 14));
		label_10.setBounds(250, 87, 464, 14);
		panel.add(label_10);
	...
	private void buttons(String profil, String nume, String CNP) {
		// buttons
		JButton button = new JButton("Inserare");
		button.setBounds(200, 427, 89, 23);
		panel.add(button);
		button.addActionListener(e -> insert());
	...
	private void insert() {
		try {
			...
			if (t.equals("profesor")) {
				CallableStatement statement = con.prepareCall("{call proiect.insertprofesor(?,?,?,?,?,?,?,?)}");
				...
				CallableStatement statement = con.prepareCall("{call proiect.insertstudent(?,?,?,?,?,?,?,?,?,?)}");
				...
				String y = textField_10.getText();
				Integer aux2 = Integer.parseInt(y);
				statement.setInt(9, aux1);
				statement.setInt(10, aux2);
				statement.execute();
				statement.close();
			}

			else if (t.equals("administrator")) {
				CallableStatement statement = con.prepareCall("{call proiect.insertadministrator(?,?,?,?,?,?,?,?)}");
